...
/// `/docs` is generated by reflecting over the config schemas, so it breaks the
/// moment a component's config stops deriving `JsonSchema`.
#[tokio::test]
async fn the_docs_page_renders_every_component_kind() -> anyhow::Result<()> {
    let req = Request::builder().uri("/docs").body(Body::empty())?;
    ...
    for component in ["dummy", "nats", "streamer", "filter", "reducer", "stdout"] {
        assert!(
            ...
            "/docs should document '{component}'"
        );
    }
    Ok(())
}
