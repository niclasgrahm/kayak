...
This is the full reference for kayak's UI and pipeline model — how the canvas
works, how a pipeline is put together, and every component kind. It's the
detail behind the [readme](../readme.md)'s quick tour; start there if you
haven't run kayak yet. For contributor-facing notes on the codebase itself
(where things live, why they're built the way they are), see
[`CLAUDE.md`](../CLAUDE.md).
