...
Every request and response body the endpoints above name, as JSON Schema generated from the Rust types they deserialize into.

## `ApiError` {#schema-apierror}

The error body every failing request comes back with.

A Rust type rather than a hand-written schema because it has to stay in step with what `AppError` actually serializes — `an_error_body_matches_the_documented_shape` in `tests/api.rs` is what says so.
...
## `BucketSummary` {#schema-bucketsummary}
...
```json
{
  "$defs": {
    "BucketSummary": {
      "description": "One bucket as the API reports it.",
      "properties": {
        "idle_timeout_secs": {
          "format": "uint64",
          "minimum": 0,
...
## `ComponentDoc` {#schema-componentdoc}
  ...
  "$defs": {
    ...
    "OutputConfig": {
      "oneOf": [
        ...
        {
          "$ref": "#/$defs/ClickhouseOutputConfig",
          "properties": {
            "type": {
              "const": "clickhouse",
              "type": "string"
...
## `LayoutFile` {#schema-layoutfile}

The whole file: every pipeline someone has placed by hand, and every edge whose route they have adjusted.

Absent ids are the normal case, not a gap to be filled — the canvas lays those out itself. A pipeline is added here the first time it is dragged and removed when the arrangement is reset.
...
An OpenAPI 3.1 document. See https://spec.openapis.org/oas/v3.1.0
...
## `PipelineHistory` {#schema-pipelinehistory}
